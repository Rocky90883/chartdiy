(function() {
    ...
    var db = {
        ...
        loadData: function(filter) {
            return $.grep(this.clients, function(client) {
                return (!filter.shopName || client.shopName.indexOf(filter.shopName) > -1)
                    && (filter.foodAmount === undefined || client.foodAmount === filter.foodAmount)
                    && (!filter.foodCount || client.foodCount.indexOf(filter.foodCount) > -1)
                    && (!filter.shopID || client.shopID === filter.shopID)
                    && (filter.reportDate === undefined || client.reportDate === filter.reportDate);
            });
        },
        // loadData: function(filter) {
        //     return $.grep(this.clients, function(client) {
    ...
    $.ajax({
        type: "get",
        dataType: "text",
        cotenType: 'application/json',
        url: "http://localhost:8080/hualala/queryByIdOrderStatus?pageNum=1&size=50&orderStatus=40&shopName=广州番禺光明南路直营店",
        async: false,
        data: {},
        success: function (mb) {
            var objdto
            objdto = JSON.parse(mb)
            console.log(objdto);
            var orderList = objdto.data.dataList
            db.clients = orderList;
        },
        error:function () {
            console.log('err');
        }
